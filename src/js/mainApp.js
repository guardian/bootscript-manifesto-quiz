...
export function init(dom, context, config, mediator) {
    ...
    var questions = context.querySelectorAll('.atom-quiz__question, .quiz__question');
    ...
    for (var i = 0, len = questions.length; i < len; i++) {
        ...
        var questionText = question.querySelector('.atom-quiz__question-text, .question__text');
        var questionAnswers = question.querySelector('.atom-quiz__answers, .question__answers');
    ...
    var labels = document.querySelectorAll('.element-atom label, .question__answer');
...
function scoreQuiz() {
    ...
    for (var q = 0; q < resultKey.length; q++) {
        ...
        var questionEl = document.querySelector('.atom-quiz__question:nth-child('+(q+1)+'), .quiz__question:nth-child('+(q+1)+')');
        for (var a = 0; a < question.length; a++) {
            var answer = resultKey[q][a];
            var answerEl = questionEl.querySelector('.atom-quiz__answer:nth-child('+(a+1)+'), .question__answer:nth-child('+(a+1)+')');
            var inputEl = answerEl.querySelector('input:checked, .highlight-answer');
            if (inputEl) {
    ...
    if (totalPoints > 2) {
        ...
        allInputs = document.querySelectorAll('.element-atom input, .element-atom .question__answer');
        for (var k = 0; k < allInputs.length; k++) {
            allInputs[k].setAttribute('disabled', 'true');
        }
